...
drs_gains_sigma2 = Dict("SVN" => 0.03430827806456538, "FRA" => 0.40466653654978035, "ALB" => 0.17598262518903063, 
                        "NLD" => 0.13611438321401437, "MNE" => 0.05321553235140275, "EST" => 0.38373155960156335, 
                        "SVK" => 0.0638809238646676, "ARM" => 0.09866103201141563, "BLR" => 0.377747485257518, 
                        "TJK" => 0.37693088684735976, "UZB" => 0.3854096742927471, "CZE" => 0.13472480245202423, 
                        "DNK" => 0.4928894123831684, "HRV" => 0.08942219592722456, "BIH" => 0.11366839259787653, 
                        "AUT" => 0.12463160718159116, "ROU" => 0.2872689594678679, "MDA" => 0.10348714216636967, 
                        "ESP" => 0.4839449331443424, "SRB" => 0.2275039584070715, "CHE" => 0.08760611098082016, 
                        "ITA" => 0.5289029947453949, "AZE" => 0.31708235865646195, "MKD" => 0.0577014079480076, 
                        "PRT" => 0.331142099206172, "HUN" => 0.1561471133811887, "BGR" => 0.2635875215140082, 
                        "KGZ" => 0.46365030164665644, "LTU" => 0.1709232947012529, "POL" => 0.42731837000531403, 
                        "TUR" => 0.5544864374439573, "GRC" => 0.5574499265345256, "DEU" => 0.3688600022312194, 
                        "UKR" => 0.42237957485928845, "BEL" => 0.0982542654821783, "LVA" => 0.22282857308826287, 
                        "TKM" => 0.6331569405219062, "GEO" => 0.17772393969932576, "XKX" => 0.059755826373497634)
...
# IRS failed with sigma2:
countries = ["GEO", "KGZ", "LTU", "LVA", "MDA", "MKD", "MNE", "NLD", "POL", "PRT", "ROU", "SRB", "SVK", "SVN", "TJK", "TKM", "TUR", "UKR", "UZB", "XKX"]
# DRS failed with sigma2:
countries = ["ITA", "LTU", "LVA", "MDA", "MKD", "MNE", "NLD", "POL", "PRT", "ROU", "SRB", "SVK", "SVN", "TJK", "TKM", "TUR", "UKR", "UZB", "XKX"]
...
for c in countries  
    ...
    res_nodes = CSV.read("results/grid_network/country/nodes_results_$(c)_realloc_fixed_irs_sigma2_rho0.csv", DataFrame) 
    res_nodes = res_nodes[.!res_nodes.is_buff, :]
    target = sum(res_nodes.uj .* res_nodes.Lj_orig) / sum(res_nodes.uj_orig .* res_nodes.Lj_orig) - 1 # irs_gains[c] / 100
    ...
    sigma = 2 # Elasticity of substitution parameter (3.8 = Armington)
    ...
    filename = "$(c)_Mrealloc_fixed_irs_sigma2_rho0" # adjust if sigma != 1.5
...
df.iso3c .= keys(Kres)
CSV.write("results/grid_network/ALL_Mrealloc_fixed_irs_sigma2_rho0.csv", df)
